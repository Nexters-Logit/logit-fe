...
export default function OpenEventPage() {
  ...
  useEffect(() => {
    if (!e01) return;
    const colors = ["#40a5ff", "#2571eb", "#c3dcff", "#fcd34d", "#f59e0b", "#a78bfa", "#ffffff"];
  ...
  useEffect(() => {
    if (!e01) { setSignupPhase(0); setDisplayCount(0); return; }
  ...
  useEffect(() => {
    if (!e02) { setCheckedDays(new Set()); setCalendarAnimating(false); return; }
    ...
    const t = setTimeout(() => {
      ...
      interval = setInterval(() => {
        ...
        day++;
        if (day >= 22) { clearInterval(interval); setCalendarAnimating(false); }
      }, 70);
    }, 300);

    return () => { clearTimeout(t); clearInterval(interval!); setCalendarAnimating(false); };
  }, [e02]);

  const onTiltMove = useCallback((ev: React.MouseEvent<HTMLDivElement>) => {
    if (!e04) return;
  ...
  const onTiltLeave = useCallback((ev: React.MouseEvent<HTMLDivElement>) => {
    const el = ev.currentTarget;
    el.style.transition = "transform 0.45s ease-out, opacity 0.6s, background 0.3s";
  ...
  return (
    <>
      <ScrollTracker sections={SECTIONS} activeId={activeId} onNavigate={navigateTo} />
      ...
      <div className="h-screen overflow-y-scroll snap-y snap-mandatory scrollbar-hide">
        ...
        >
            ...
            >
              {coin ? (
                <div style={{
                  ...
                  boxShadow: "inset 0 2px 3px rgba(255,255,255,0.6), inset 0 -1px 2px rgba(0,0,0,0.15), 0 3px 8px rgba(120,53,15,0.3)",
                ...
                <Zap style={{ width: s, height: s }} className="text-primary-200" strokeWidth={1.5} fill="currentColor" />
              )}
            </div>
          ))}

          <div className="flex items-center gap-2">
            <Image src="/logos/logo-symbol-2d.svg" alt="Logit" width={32} height={32} />
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-body-5-4 text-gray-200">신규 가입 · 매일 출석 · 친구 초대 · 요금제 할인</p>
          </div>
          {/* D-day 카운트다운 */}
          <div className="flex flex-col items-center gap-2">
            <p className="text-body-9-3 text-gray-200/70">2026.06.13 ~ 2026.08.13</p>
            <p className="text-body-8-1 text-gray-200 tracking-widest">이벤트 종료까지</p>
            <div className="flex items-end gap-1.5">
              {[
                ...
                <div key={u} className="flex items-end gap-1.5">
                  {idx > 0 && <span className="text-body-5-1 text-gray-100 mb-6">:</span>}
        ...
        >
          ...
          <div className={`w-full max-w-96 flex flex-col items-center text-center gap-5 relative ${EVENT_STATUS.signup ? "opacity-25 pointer-events-none select-none" : ""}`}>
            ...
            <div
              ...
              style={{ opacity: e01 ? 1 : 0, transform: e01 ? "none" : "translateY(14px)" }}
            >
              <p className="text-title-1 text-gray-500 font-bold tracking-tight">AI 자소서, 무료로 시작</p>
              <div className="flex gap-2 flex-wrap justify-center">
                {[
                  ...
                  <span key={label} className="flex items-center gap-1 px-3 py-1 rounded-full bg-primary-200/15 border border-primary-200/20 text-body-8-1 text-primary-200">
            ...
            <div
              ...
              style={{ opacity: e01 ? 1 : 0, transform: e01 ? "none" : "translateY(20px) scale(0.96)" }}
            >
              {/* 잔액 헤더 */}
              <div className="bg-primary-50 px-6 pt-6 pb-4 text-center">
                <p className="text-body-9-1 text-primary-100 mb-2 tracking-wider">내 토큰 잔액</p>
                <span className="text-display-1 text-primary-200 tabular-nums animate-blue-glow">{displayCount}</span>
              </div>
              ...
              <div className="relative h-2 bg-primary-50">
                ...
                <div
                  ...
                  style={{ left: "50%", width: `${Math.max(0, displayCount - 50)}%` }}
                />
              </div>
              ...
              <div className="px-6 pt-4 pb-2 flex flex-col">
                <div
                  ...
                  style={{ opacity: signupPhase >= 1 ? 1 : 0, transform: signupPhase >= 1 ? "none" : "translateY(8px)" }}
                >
                  <div className="flex items-center gap-2">
                    <div className="size-5 rounded-full bg-primary-50 flex items-center justify-center shrink-0">
                      <Check className="size-3 text-primary-100" strokeWidth={3} />
                    </div>
                    <span className="text-body-7-3 text-gray-200">가입 보너스</span>
                  </div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-title-3 text-primary-100 tabular-nums font-bold animate-blue-glow">+50</span>
                    <span className="text-body-7-2 text-primary-100">토큰</span>
                  </div>
                </div>
                <div
                  className="flex items-center justify-between py-3 border-b border-primary-50 transition-all duration-500"
                  style={{ opacity: signupPhase >= 2 ? 1 : 0, transform: signupPhase >= 2 ? "none" : "translateY(8px)" }}
                >
                  <div className="flex items-center gap-2">
                    <div className="size-5 rounded-full bg-primary-50 flex items-center justify-center shrink-0">
                      <Check className="size-3 text-primary-100" strokeWidth={3} />
                    </div>
                    <span className="text-body-7-3 text-gray-200">월 기본 지급</span>
                  </div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-title-3 text-primary-200 tabular-nums font-bold animate-blue-glow">+50</span>
                ...
                <div
                  ...
                  style={{ opacity: signupPhase >= 3 ? 1 : 0, transform: signupPhase >= 3 ? "none" : "translateY(8px)" }}
                >
                  <span className="text-body-6-2 text-gray-300">합계</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-title-2 text-primary-200 tabular-nums font-bold">100</span>
            ...
            <div
              ...
              style={{ opacity: e01 ? 1 : 0 }}
            >
              <p className="text-body-7-3 text-gray-300">별도 신청 없이 가입 즉시 자동 지급돼요</p>
              <p className="text-body-7-3 text-gray-200">채팅 5토큰 · 초안 생성 10토큰 소모</p>
            </div>
          </div>
        ...
        >
          ...
          <div className={`w-full max-w-96 flex flex-col items-center text-center gap-5 relative ${EVENT_STATUS.attendance ? "opacity-25 pointer-events-none select-none" : ""}`}>
            ...
            <div
              ...
              style={{ opacity: e02 ? 1 : 0, transform: e02 ? "none" : "scale(0.7)" }}
            >
              <p className="text-body-8-1 text-white/40 tracking-widest uppercase">하루 출석 보상</p>
              <div className="flex items-baseline gap-2">
                <span className="text-display-1 text-white tabular-nums animate-white-glow">+3</span>
            ...
            >
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20">
                <span className="text-body-5-1 text-white tabular-nums">{checkedDays.size * 3} 토큰</span>
                <div className="w-px h-3.5 bg-white/20" />
                <span className="text-body-7-3 text-white/50">{checkedDays.size}일 누적</span>
              </div>
              <div className="flex items-baseline gap-1">
                <span className="text-body-8-3 text-white/40">개근 시 최대</span>
              ...
              {Array.from({ length: 35 }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    style={isChecked ? { background: "rgba(255, 247, 230, 0.95)" } : {}}
                  >
                    {isChecked ? (
                      <div
                        ...
                        style={{
                          ...
                          background: "linear-gradient(145deg, #fcd34d 0%, #f59e0b 55%, #d97706 100%)",
                          boxShadow: "inset 0 1.5px 2px rgba(255,255,255,0.55), 0 2px 4px rgba(120,53,15,0.3)",
                          border: "1px solid rgba(251,191,36,0.5)",
                          display: "flex", alignItems: "center", justifyContent: "center",
                        }}
                      >
                        <span style={{ fontSize: "9px", fontWeight: 900, color: "#78350f", lineHeight: 1, letterSpacing: "-0.3px" }}>+3</span>
                      </div>
                    ) : isClickable ? (
                      <div
                      ...
                      >
                        <span style={{ fontSize: "9px", fontWeight: 900, color: "rgba(252,211,77,0.6)", lineHeight: 1 }}>+3</span>
                      </div>
                    ) : null}
                  </button>
                );
            ...
            >
              ...
              <div className="w-full flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-body-8-2 text-white/40">이벤트 풀 소진</span>
                  <span className="text-body-8-1 text-white/70">
                    {POOL_REMAINING.toLocaleString()} / {POOL_TOTAL.toLocaleString()} 남음
                  </span>
                </div>
                ...
                <p className="text-body-9-3 text-white/30 text-right">
                  {Math.round((POOL_TOTAL - POOL_REMAINING) / POOL_TOTAL * 100)}% 소진됐어요
                </p>
              </div>
              <Link
                href="/event/attendance"
                className="mt-1 w-full block text-center py-4 rounded-3.5 bg-white text-primary-200 text-body-6-1 hover:bg-primary-50 transition-colors"
              >
                출석하러 가기
              </Link>
            </div>
        ...
        >
          ...
          <div className={`w-full max-w-120 flex flex-col items-center text-center gap-8 relative ${EVENT_STATUS.referral ? "opacity-25 pointer-events-none select-none" : ""}`}>
            ...
            <div
              className="flex flex-col items-center gap-3 transition-all duration-700 delay-75"
              style={{ opacity: e03 ? 1 : 0, transform: e03 ? "scale(1)" : "scale(0.7)" }}
            >
              <div className="flex items-baseline gap-2 justify-center">
                <span className="text-display-2 text-white animate-white-glow">10</span>
                <span className="text-title-2 text-white/80 font-bold">토큰씩</span>
              </div>
              <div className="flex gap-2 flex-wrap justify-center">
                {["둘 다 지급", "즉시 자동 지급", "인원 무제한"].map((text) => (
                  <span key={text} className="flex items-center gap-1 px-3 py-1 rounded-full bg-white/8 border border-white/12 text-body-8-1 text-white/65">
                    <Check className="size-3 shrink-0 text-primary-100" strokeWidth={2.5} />
                    {text}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-start gap-6 w-full max-w-96">
              ...
              <div className="flex-1 flex flex-col gap-8 mt-5">
                ...
                <div className="flex items-center">
                  <div className="relative flex-1 overflow-hidden" style={{ height: 12 }}>
                    {/* 베이스 트랙 */}
                    <div className="absolute left-0 right-0 bg-white/15" style={{ top: "50%", height: 2, transform: "translateY(-50%)" }} />
                    {/* 공통 wrapper — 글로우+빔이 같은 부모를 공유해 항상 동기화 */}
                    <div className="absolute" style={{
                      top: 0, height: "100%", width: "100%",
                      animation: e03 ? "seq-beam-right 4s cubic-bezier(0.4,0,0.55,1) infinite" : "none",
                    }}>
                      <div style={{
                        position: "absolute", top: "50%", transform: "translateY(-50%)",
                        width: "100%", height: 12,
                        background: "radial-gradient(ellipse at 95% 50%, rgba(64,165,255,0.7) 0%, rgba(64,165,255,0.2) 22%, transparent 48%)",
                        filter: "blur(3px)",
                      }} />
                      <div style={{
                        position: "absolute", top: "50%", transform: "translateY(-50%)",
                        width: "100%", height: 2,
                        background: "linear-gradient(to right, transparent 0%, rgba(64,165,255,0.1) 15%, rgba(64,165,255,0.7) 52%, rgba(64,165,255,1) 78%, rgba(255,255,255,1) 93%, transparent 100%)",
                      }} />
                    </div>
                  </div>
                  <div style={{ width: 0, height: 0, borderTop: "4px solid transparent", borderBottom: "4px solid transparent", borderLeft: "6px solid rgba(255,255,255,0.28)", flexShrink: 0, animation: e03 ? "arrow-glow-right 4s ease-in-out infinite" : "none" }} />
                </div>

                {/* 트랙 2: 친구 → 나 */}
                <div className="flex items-center">
                  <div style={{ width: 0, height: 0, borderTop: "4px solid transparent", borderBottom: "4px solid transparent", borderRight: "6px solid rgba(255,255,255,0.28)", flexShrink: 0, animation: e03 ? "arrow-glow-left 4s ease-in-out infinite" : "none" }} />
                  <div className="relative flex-1 overflow-hidden" style={{ height: 12 }}>
                    <div className="absolute left-0 right-0 bg-white/15" style={{ top: "50%", height: 2, transform: "translateY(-50%)" }} />
                    <div className="absolute" style={{
                      top: 0, height: "100%", width: "100%",
                      animation: e03 ? "seq-beam-left 4s cubic-bezier(0.4,0,0.55,1) infinite" : "none",
                    }}>
                      <div style={{
                        position: "absolute", top: "50%", transform: "translateY(-50%)",
                        width: "100%", height: 12,
                        background: "radial-gradient(ellipse at 5% 50%, rgba(64,165,255,0.7) 0%, rgba(64,165,255,0.2) 22%, transparent 48%)",
                        filter: "blur(3px)",
                      }} />
                      <div style={{
                        position: "absolute", top: "50%", transform: "translateY(-50%)",
                        width: "100%", height: 2,
                        background: "linear-gradient(to left, transparent 0%, rgba(64,165,255,0.1) 15%, rgba(64,165,255,0.7) 52%, rgba(64,165,255,1) 78%, rgba(255,255,255,1) 93%, transparent 100%)",
                      }} />
                    </div>
                  </div>
                </div>
              ...
              >
                <span
                  className="absolute -top-7 left-1/2 -translate-x-1/2 text-body-6-1 text-primary-100 font-bold"
                  style={{ animation: e03 ? "seq-token-right 4s ease-out infinite" : "none" }}
                >+10</span>
                <div
                  className="size-16 rounded-full bg-white/8 border border-white/15 flex items-center justify-center"
                  style={{ animation: e03 ? "seq-glow-phase1 4s ease-out infinite" : "none" }}
                >
        ...
        >
          ...
          <div className={`w-full max-w-160 flex flex-col items-center text-center gap-6 relative ${EVENT_STATUS.discount ? "opacity-25 pointer-events-none select-none" : ""}`}>
            ...
            <div
              ...
              style={{ opacity: e04 ? 1 : 0, transform: e04 ? "none" : "translateY(16px)" }}
            >
            ...
            <div className="w-full flex flex-wrap sm:flex-nowrap gap-3 items-stretch">
              ...
              >
                <span className="text-body-9-1 text-white/40 tracking-wider">MCP</span>
                <div className="flex flex-col gap-1">
                  <div className="relative self-start">
                    <span className="text-body-8-2 text-white/40">2,990원</span>
                    <div style={{ position: "absolute", left: 0, top: "50%", height: 1, width: "100%", background: "rgba(255,255,255,0.5)", transformOrigin: "left center", transform: e04 ? "translateY(-50%) scaleX(1)" : "translateY(-50%) scaleX(0)", transition: e04 ? "transform 0.35s ease-out 1.0s" : "none" }} />
                  </div>
                  <div className="flex items-baseline gap-1" style={{ opacity: e04 ? 1 : 0, transform: e04 ? "none" : "translateY(6px)", transition: e04 ? "opacity 0.4s 1.5s, transform 0.4s 1.5s" : "none" }}>
                    <span className="text-title-2-2 text-white font-bold">1,000원</span>
                    <span className="text-body-9-3 text-white/40">/월</span>
                  </div>
                </div>
                <div className="h-px bg-white/10" />
                <div className="flex flex-col gap-1.5 text-left">
                  <p className="text-body-8-2 text-white/50 sm:hidden">AI 어시스턴트</p>
                  <div className="hidden sm:flex flex-col gap-1.5">
                    <p className="text-body-8-2 text-white/50">AI 어시스턴트 연동</p>
              ...
              <div
                ...
                style={{
                  opacity: e04 ? 1 : 0,
                  transform: e04 ? "none" : "translateY(32px) scale(0.95)",
                  transition: "opacity 0.6s 0.35s, transform 0.6s 0.35s, background 0.3s",
                  willChange: "transform",
                }}
              >
                ...
                <span className="text-body-9-1 text-primary-70 tracking-wider">Light</span>
                <div className="flex flex-col gap-1">
                  <div className="relative self-start">
                    <span className="text-body-8-2 text-white/40">9,900원</span>
                    <div style={{ position: "absolute", left: 0, top: "50%", height: 1, width: "100%", background: "rgba(255,255,255,0.5)", transformOrigin: "left center", transform: e04 ? "translateY(-50%) scaleX(1)" : "translateY(-50%) scaleX(0)", transition: e04 ? "transform 0.35s ease-out 1.15s" : "none" }} />
                  </div>
                  <div className="flex items-baseline gap-1" style={{ opacity: e04 ? 1 : 0, transform: e04 ? "none" : "translateY(6px)", transition: e04 ? "opacity 0.4s 1.65s, transform 0.4s 1.65s" : "none" }}>
                    <span className="text-title-2-2 text-white font-bold">6,900원</span>
                    <span className="text-body-9-3 text-white/40">/월</span>
                  </div>
                </div>
                <div className="h-px bg-white/15" />
                <div className="flex flex-col gap-1.5 text-left">
                  <p className="text-body-8-1 text-primary-70 font-semibold">월 400토큰</p>
                  <div className="hidden sm:flex flex-col gap-1">
                    <p className="text-body-8-2 text-white/50">채팅 80회 상당</p>
                    <p className="text-body-8-2 text-white/50">초안 40회 상당</p>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                style={{
                  opacity: e04 ? 1 : 0,
                  transform: e04 ? "none" : "translateY(32px) scale(0.95)",
                  transition: "opacity 0.6s 0.5s, transform 0.6s 0.5s, background 0.3s",
                  willChange: "transform",
                }}
              >
                <span className="text-body-9-1 text-white/40 tracking-wider">Pro</span>
                <div className="flex flex-col gap-1">
                  <div className="relative self-start">
                    <span className="text-body-8-2 text-white/40">19,900원</span>
                    <div style={{ position: "absolute", left: 0, top: "50%", height: 1, width: "100%", background: "rgba(255,255,255,0.5)", transformOrigin: "left center", transform: e04 ? "translateY(-50%) scaleX(1)" : "translateY(-50%) scaleX(0)", transition: e04 ? "transform 0.35s ease-out 1.3s" : "none" }} />
                  </div>
                  <div className="flex items-baseline gap-1" style={{ opacity: e04 ? 1 : 0, transform: e04 ? "none" : "translateY(6px)", transition: e04 ? "opacity 0.4s 1.8s, transform 0.4s 1.8s" : "none" }}>
                    <span className="text-title-2-2 text-white font-bold">14,900원</span>
                    <span className="text-body-9-3 text-white/40">/월</span>
                  </div>
                </div>
                <div className="h-px bg-white/10" />
                <div className="flex flex-col gap-1.5 text-left">
                  <p className="text-body-8-1 text-white/80 font-semibold">월 2,000토큰</p>
                  <div className="hidden sm:flex flex-col gap-1">
                    <p className="text-body-8-2 text-white/50">채팅 400회 상당</p>
                    <p className="text-body-8-2 text-white/50">초안 200회 상당</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              style={{ opacity: e04 ? 1 : 0, transform: e04 ? "none" : "translateY(12px)" }}
            >
              <Link
                href="/profile/plans"
          ...
          <div className="w-full max-w-120 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={i} className="flex gap-3 items-start">
                  ...
                  <p className="text-body-7-3 text-white/40 leading-relaxed">{text}</p>
                </div>
              ))}
            </div>
            ...
            <div className="pt-6 border-t border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-2 opacity-40">
                <Image src="/logos/logo-symbol-2d.svg" alt="Logit" width={16} height={16} />
